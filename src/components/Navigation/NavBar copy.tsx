...
export default function NavBar({ className }: Props) {
  ...
  const navTos = navItems.map((item) => `/${item.toLowerCase()}`)
  navTos[0] = "/"
  ...
  return (
    <header
      className={cn(
        " fixed top-0 w-full bg-bg text-text py-2 xs:py-4 px-4 min-h-14 border-b-2 border-bg-3 z-30",
        className
      )}
    >
      <nav className="flex justify-between items-center flex-wrap  w-full">
        ...
        <ul
          className={cn(
            "order-2 xs:order-none font-normal flex flex-col mt-4 w-full xs:w-fit xs:flex-row xs:space-x-4 xs:mt-0 sm:space-x-8",
            "transition-all duration-300 ease-in overflow-hidden",
            visible ? "max-h-60" : "max-h-0",
            "xs:max-h-60"
          )}
        >
